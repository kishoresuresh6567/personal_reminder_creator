...
describe("App", () => {
  ...
  beforeEach(() => {
    ...
    saveAudioReminderMock.mockResolvedValue({
      ...
      transcriptError: "Cloud transcription failed",
      transcribedAt: null,
    });
  ...
  it("removes the alarm when a one-time reminder is completed", async () => {
    const user = userEvent.setup();
  ...
  it("updates the alarm view after a weekly reminder reschedule", async () => {
    const user = userEvent.setup();
    ...
    await user.selectOptions(screen.getByLabelText(/weekly time/i), "14:30");
    await user.click(screen.getByRole("button", { name: /confirm reschedule/i }));
    expect(await screen.findByLabelText(/reschedule success/i)).toHaveTextContent(/rescheduled/i);
  ...
  it("updates the alarm view after a monthly reminder reschedule", async () => {
    const user = userEvent.setup();
    ...
    expect(screen.getAllByText(/2 Feb 2099/i).length).toBeGreaterThan(0);
  });
  ...
  it("saves non-empty recorded audio when recording stops", async () => {
    ...
    expect(screen.getByLabelText(/transcript status/i)).toHaveTextContent(/cloud transcription failed/i);
    expect(createReminderFromTranscriptMock).not.toHaveBeenCalled();
    expect(screen.getByLabelText(/reminder status/i)).toHaveTextContent(/no transcript was captured/i);
    expect(stop).toHaveBeenCalledTimes(1);
  });

  it("creates a reminder from the server-transcribed audio", async () => {
    const user = userEvent.setup();
    const { stream } = createMediaStreamMock();
  ...
  it("shows an empty status when server transcription detects no speech", async () => {
    const user = userEvent.setup();
    const { stream } = createMediaStreamMock();
    ...
    await waitFor(() => expect(screen.getByText(/audio saved/i)).toBeInTheDocument());
    expect(saveAudioReminderMock).toHaveBeenCalledWith(expect.objectContaining({ blob: expect.any(Blob) }));
    expect(createReminderFromTranscriptMock).not.toHaveBeenCalled();
    expect(screen.getByLabelText(/transcript status/i)).toHaveTextContent(/no transcript captured/i);
  });

  it("shows a failed status when server transcription fails", async () => {
    const user = userEvent.setup();
    const { stream } = createMediaStreamMock();
